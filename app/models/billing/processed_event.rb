module Billing
  # Idempotency ledger for Stripe webhooks. The unique index on event_id makes
  # #record_once the single arbiter of "have we handled this delivery before?", so
  # a Stripe replay produces exactly one effect.
  class ProcessedEvent < ApplicationRecord
    ...
    # retry can reprocess it (no silently-lost webhook).
    ...
    # Its own savepoint for two reasons: a losing race must not poison the
    # surrounding transaction, and the RecordNotUnique rescue must not reach the
    # caller's block. Rescuing the whole block made a unique violation raised by the
