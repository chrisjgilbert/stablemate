...
class Project::MonitorSyncTest < ActiveSupport::TestCase
  ...
  test "cannot mass-assign protected attributes through the payload" do
    ...
    result = @project.sync_monitors(entries: [
      { registration_key: "evil", name: "Evil", expected_interval_seconds: 60,
        ...
        project_id: other.id, user_id: users(:alice).id }
    ])
    monitor = result[:registered].first
  ...
  # --- F2: the repo owns monitor config -------------------------------------

  # The one survivor of the ten arbitration tests that used to live here
  # (v1-scope §3.1): it asserts exactly what always-write does, so it reads the
  # same before and after. Its nine siblings asserted a refusal or the
  # `last_synced_*` columns and went with them — the round trip and the recovery
  # property that replace them live in monitor_sync_always_write_test.rb.
  ...
  # F2 x F7: next_due_at is re-derived whenever the interval changes. Under
  # arbitration this test asserted the opposite — that a REFUSED interval left
  # the due time alone — which was the same property read from the other side:
  # the detection window must never drift away from the interval actually in
  # force. Always-write keeps that invariant and inverts the case, because now
  # the sync's interval is always the one in force.
  test "a synced interval change re-derives the detection window" do
    @project.sync_monitors(entries: [ entry("digest", interval: 3600, grace: 300) ])
    monitor = @project.monitors.find_by(registration_key: "digest")
    monitor.register_contact(Time.current)
    monitor.save!

    @project.sync_monitors(entries: [ entry("digest", interval: 900, grace: 300) ])
