class RegistrationsController < ApplicationController
  ...
  # #create enqueues a verification email to a caller-supplied address, so an
  # unthrottled endpoint is an email-bombing / mass-account vector. Dedicated
  # in-process store so the bound holds under the test env's null_store.
  ...
  private
    def join_waitlist(signup)
      if signup.persisted?
        ...
        # Errors are generic — "can't be blank" — so this never becomes an
        # enumeration oracle for who is already on the list; a duplicate is a
        # success above.
