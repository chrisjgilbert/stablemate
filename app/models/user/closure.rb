class User
  # Closing an account for good, reached via user.close_account!. It exists for the
  # ORDER of its steps:
  #
  #   1. Cancel Stripe FIRST, so a Stripe failure aborts with the account intact
  #      rather than half-closed. It also defuses Pay's own `after_commit
  #      :cancel_active_pay_subscriptions!, on: [:destroy]`, which would otherwise
  #      fire a live Stripe call after the user row is gone.
  ...
  #      later let in. The privacy policy promises deletion removes every trace, so
