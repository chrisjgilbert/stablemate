...
# dormant unless Stripe keys are configured (Stablemate.billing_enabled?), which
# is the self-host default: no keys ⇒ Pay tables stay empty and everyone is Free.
...
#
#   * POST /pay/webhooks/stripe — a SECOND Stripe webhook endpoint that verifies
#     the same signature but bypasses Billing::ProcessedEvent idempotency, the
#     livemode gate and the plan sync. Billing::WebhooksController is the only
#     Stripe entry point (PRD §12: one writer of User.plan).
#   * GET /pay/payments/:id — unauthenticated, and embeds the PaymentIntent's
#     client_secret. We use Stripe Checkout and never need it.
#
# Must be set before Pay::Engine's "pay.processors" initializer appends the
# mount; app initializers run first, so here is early enough.
...
Pay.setup do |config|
  ...
  # it here overrides ours with a domain a self-hoster doesn't own. Leaving it
  # unset makes Pay follow STABLEMATE_MAIL_FROM (MailFromTest pins this).

  # One paid product — Pro. Naming every subscription "pro" lets
  # User::Subscription#subscribed_to_pro? ask Pay a single, plan-agnostic question.
  config.default_product_name = "pro"

  # No stock Pay emails (D9): they default on, are entirely unconfigured, and
  ...
  # re-sending the mail each time. Every message we send is our own, queued, and
  # sent after commit. To re-enable: drop this line, brand the templates first,
  # and move the payment_failed delivery off the webhook's transaction.
