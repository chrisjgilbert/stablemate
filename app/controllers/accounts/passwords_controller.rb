module Accounts
  ...
  # custom verb, and deliberately a different controller from the top-level
  # PasswordsController, which is the *unauthenticated* "I forgot it" token-reset
  # flow. The two share nothing: this one proves ownership with the current
  # password, that one with a signed emailed token.
  #
  # Session policy on success: this session survives, every OTHER session is signed
  # out. Changing your password is how you evict someone who has your cookie, so
  # the other sessions have to die — but re-prompting the person who just typed
  # both passwords proves nothing.
  class PasswordsController < ApplicationController
    ...
    def update
      ...
      # Guard the PERMITTED password, not the raw param — both halves of that bug
      # (a blank password, and a non-scalar one) are spelled out on
