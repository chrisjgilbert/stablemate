...
| 6 | ActiveJob → monitor mapping key | **Solid Queue task key** — `registration_key` = the `recurring.yml` task key. The registrar (Layer 2) writes it; the execution subscriber (Layer 1) resolves a job's `perform.active_job` back to that key. **Amended by [`projects.md`](projects.md):** its uniqueness is now scoped **per project** — `(project_id, registration_key)` — since a monitor belongs to a project rather than directly to a user (the multi-app collision fix). |
...
### `Project`
`id`, `user_id`, `name`, timestamps. A first-class grouping (see
[`projects.md`](projects.md)): a user has many projects; a project owns its
monitors and API keys. Ownership flows `monitor → project → user`, so the cap and
alerts stay per-user while gem identity + the `registration_key` namespace become
per-project. Indexes: `(user_id, name)` unique; `user_id`.
...
- `last_synced_app` records the free-text `app` the gem sends on sync (advisory);
  a divergence on a later sync flags a shared-key collision (`projects.md` §3.2).
- Indexes: `ping_token` (unique); `(project_id, registration_key)` unique **where
